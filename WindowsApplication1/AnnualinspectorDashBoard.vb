...
Public Class AnnualinspectorDashBoard
            ...
            Try
                ...
                If rdr_ms.Read = True Then
                    ...
                    With Inspector_ApplicationRecord
                        ...
                        .TxtBusinessAddress.Text = rdr_ms("bldg_age").ToString
                        .TxtBusinessOwner.Text = rdr_ms("bldg_age").ToString
                ...
                conn_ms = "SELECT * FROM ONLINE.annual_inspection_application where id= '" & DataGrid.Item(0, DataGrid.CurrentRow.Index).Value & "' "
                Con_ms = New SqlConnection(mcs)
                Con_ms.Open()
                ...
                If rdr_ms.Read = True Then
                    ...
                    Dim Constr_Application As Inspector_ApplicationRecord = CType(Application.OpenForms("Inspector_ApplicationRecord"), Inspector_ApplicationRecord)
                    With Constr_Application
                        ...
                        ElseIf rdr_ms("app_status") = "R" Then
                            ...
                            .btnDeny.Visible = False
                        End If
                        .txt_remarks.Text = rdr_ms("remark").ToString
                        ...
                        If rdr_ms("app_status") = "A" Then

                            .assessment_file.Enabled = False
                            .B_assessAttach.Enabled = False
                            .B_Reupload.Enabled = False
                            .TxtBusinessAddress.Enabled = False
                            .TxtRefenceNo.Enabled = False
                            .TxtAccountNo.Enabled = False

                            .TxtBusinessAddress.Text = rdr_ms("assessmentamount").ToString()
                            .assessment_file.Text = rdr_ms("assessmentfile").ToString()


                            If Not String.IsNullOrEmpty("assessmentfile") Then
    ...
    Private Sub whoisthisuser()
        ...
        If rdr_ms2.Read = True Then
            ...
            takngaran = rdr_ms2("Fullname").ToString
    ...
    Private Sub Button8_Click_1(sender As Object, e As EventArgs) Handles Button8.Click
        ...
        Try
            If cmb_appointmentstatus.Text = "ALL" Then
                ...
                conn_ms = "SELECT * " &
                          "FROM ONLINE.annual_inspection_application " &
                          "WHERE app_status IN ('P', 'R', 'D', 'A') AND (" &
                          "(app_status IN ('P', 'R') AND CONVERT(date, application_date) BETWEEN '" & Format(dt_Appoinment.Value, "yyyy-MM-dd") & "' AND '" & Format(dt_Appoinment1.Value, "yyyy-MM-dd") & "') " &
                          ...
                          "OR (app_status = 'D' AND CONVERT(date, deny_date) BETWEEN '" & Format(dt_Appoinment.Value, "yyyy-MM-dd") & "' AND '" & Format(dt_Appoinment1.Value, "yyyy-MM-dd") & "')) " &
            ...
            ElseIf cmb_appointmentstatus.Text = "PENDING" Then
                ...
                Do While rdr_ms.Read = True
                    ...
                    Dim applicationDate As DateTime = Convert.ToDateTime(rdr_ms("application_date"))
                    ...
                    DataGrid.Rows.Add(rdr_ms("id"), rdr_ms("refNo"), rdr_ms("application_date"), rdr_ms("tdn"), rdr_ms("project_title"), pendingText, rdr_ms("app_status"), "VIEW")
            ...
            ElseIf cmb_appointmentstatus.Text = "ASSESSED" Then
                ...
                conn_ms = "SELECT *" & _
                          "FROM ONLINE.annual_inspection_application " & _
                          "WHERE app_status ='A' and Convert(date, application_date) between '" & Format((dt_Appoinment.Value), "yyyy-MM-dd") & "' and '" & Format((dt_Appoinment1.Value), "yyyy-MM-dd") & "' ORDER BY application_date ASC"
                Con_ms = New SqlConnection(mcs)
                Con_ms.Open()
                cmd_ms = New SqlCommand(conn_ms, Con_ms)
                rdr_ms = cmd_ms.ExecuteReader(CommandBehavior.CloseConnection)
            ...
            ElseIf cmb_appointmentstatus.Text = "DENIED" Then
                ...
                conn_ms = "SELECT * " & _
                          "FROM ONLINE.annual_inspection_application " & _
                          "WHERE app_status ='D' and Convert(date, application_date) between '" & Format((dt_Appoinment.Value), "yyyy-MM-dd") & "' and '" & Format((dt_Appoinment1.Value), "yyyy-MM-dd") & "' ORDER BY application_date ASC"
                Con_ms = New SqlConnection(mcs)
                Con_ms.Open()
                cmd_ms = New SqlCommand(conn_ms, Con_ms)
                rdr_ms = cmd_ms.ExecuteReader(CommandBehavior.CloseConnection)
            ...
            ElseIf cmb_appointmentstatus.Text = "RE-UPLOAD" Then
                ...
                conn_ms = "SELECT * " & _
                          "FROM ONLINE.annual_inspection_application " & _
                          "WHERE app_status ='R' and Convert(date, application_date) between '" & Format((dt_Appoinment.Value), "yyyy-MM-dd") & "' and '" & Format((dt_Appoinment1.Value), "yyyy-MM-dd") & "' ORDER BY application_date ASC"
                Con_ms = New SqlConnection(mcs)
                Con_ms.Open()
                cmd_ms = New SqlCommand(conn_ms, Con_ms)
                rdr_ms = cmd_ms.ExecuteReader(CommandBehavior.CloseConnection)
    ...
    Private Sub PictureBox2_Click(sender As Object, e As EventArgs) Handles PictureBox2.Click
        ...
        Try
            conn_ms = "SELECT * " &
              "FROM ONLINE.annual_inspection_application " &
              "WHERE app_status = 'R' ORDER BY application_date"
